...
@router.get("/tree")
async def list_tree(
    path: str = Query("", description="Relative path from workspace"),
...
def _build_tree(dir_path: Path, base: Path, depth: int) -> list[dict]:
    if depth <= 0:
        return []
    entries: list[dict] = []
    try:
        for child in sorted(dir_path.iterdir()):
            ...
            if is_dir and depth > 1:
                entry["children"] = _build_tree(child, base, depth - 1)
