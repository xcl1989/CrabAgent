...
不需要规则配置，不需要正则匹配。LLM 智能识别，自动区分真正待办和垃圾邮件。
...
**通知联动：** 收到邮件时，摘要推送到微信 *同时注入完整上下文* — 你可以直接说"回那封邮件"，Agent 知道发谁、回什么。
...
```
> 读取 sales.xlsx 汇总 Q1 数据
> 创建一个项目方案文档
> 把第 3 页标题改为"Q2 路线图"
```
文档面板在聊天旁打开——拖拽调整大小，一键全屏预览。
...
```
当前会话：中文      →  /language English  ✓ 重建 system prompt
当前会话：English   →  /language 中文      ✓ 重建 system prompt
```

- 语言偏好跨会话持久保存
- 整个 UI、CLI 提示词和 Agent 回复都会适配你选择的语言
- 支持按会话切换语言，适合多语言团队协作
- 零性能损失——system prompt 的静态部分保持缓存
