...
> **AI 知识工作平台** — 需要答案时对话，需要成果时工作，任务在桌面时让 AI 直接操作。一个 Agent，贯通终端、浏览器与原生 macOS 应用。
...
| 🌐 **浏览器** | 真实嵌入式浏览器，共享登录态；AI 导航和交互，你处理验证码和 MFA | 点击"协作浏览器" |
| 🖥️ **macOS 控制** | AI 观察和操作本机 macOS 应用——辅助功能树、窗口截图、键鼠输入。默认关闭，仅限允许列表内的应用 | 设置 → macOS 控制 |
...
- **文件工作区**：不离开工作模式即可新建文件夹、多选项目、拖放移动文件或文件夹、使用浮动右键菜单，并一键复制文本预览内容
- **协作浏览器**：AI 在嵌入式浏览器中导航网站、填写表单、提取数据，具备页面版本安全、敏感字段拦截和高风险点击拦截——你始终保有登录态并处理仅限人工的步骤
...
**成本控制：** 在「设置 → 通用」中，把主 Agent 的 `provider|模型` 映射到子 Agent 使用的（通常更便宜的）`provider|模型`，每次委派自动生效，支持跨 Provider。
...
### 🎯 目标模式
通过靶心按钮或 `/goal <目标>` 为当前会话创建持续目标。目标会始终展示目标描述、验收条件、约束、检查点和验证证据；规划、工具调用、测试与结果仍完整保留在普通对话流中。

随时可以暂停、继续、编辑或查看目标进展时间线。开启**自动继续**后，CrabAgent 会在当前回复结束后启动下一轮聚焦执行；目标完成、暂停、阻塞或达到 Token / 回合预算时会安全停止。

### 🛠️ 工作模式
分屏工作区：实时文档预览、代码编辑器、原型构建器、会议记录、Markdown 编辑器。AI 对话侧边栏全程保持交互。

### 📈 用量洞察
按时间范围和工作区追踪 Token 消耗。用量页突出显示周期消耗、调用次数、缓存表现和活跃会话，并可对比模型与 Agent 的消耗，搜索、排序会话，展开查看最近调用摘要。

### 📊 对话富可视化
AI 回复可将 Markdown 转换为可分享的可视化内容：Mermaid 流程图、时序图、状态图、ER 图、架构图，以及柱状图、折线图、面积图、饼图和散点图；KPI 卡片可突出关键指标。任意图表均可放大查看、复制为 PNG 或下载高清 PNG，方便直接分享。

Mermaid 图表会在流式回复完成后再渲染，并在显示前校验语法，避免不完整图表文本直接显示为原始解析错误图。
...
### 🖼️ 多模态
可粘贴/拖拽图片到对话，也可让 Agent 通过 `read` 读取工作区已有图片。视觉模型会直接接收图片（大图自动缩放），普通图片查看不再绕行通用图片分析 MCP；OCR、UI、技术图、数据图、界面对比和视频等专业工具仍然可用。纯文本模型（如 kimi-k2、moonshot-v1）下图片自动降级为文本占位符，会话不中断。AI 生成的图片可跨会话持久化，内联渲染。**懒加载**：图片在文本渲染后按需加载，打开图片密集的会话瞬间完成。AI 图像编辑（`image_edit`）可基于附带参考图生成新图——更换风格或背景、将同一主体放入新场景；模型 slug 从服务商动态解析，服务商侧模型变动无需更新客户端。
...
- **按窗口截图** — ScreenCaptureKit 单窗口 JPEG，绝不全屏捕获
- **输入** — CGEvent 点击、Unicode 文本（中日韩 + emoji）、命名键、修饰键组合（`cmd+s`）、滚动

**安全模型** — 输入默认关闭，由四层独立防线把关：设置里的 macOS 控制开关、按应用允许列表（未列表应用触发确认卡片，批准后自动入列）、每次动作前的前台应用校验、Secure Input / 锁屏拒绝。`Cmd+Q` / `Cmd+Tab` 被拒绝。预算限制（60 动作 / 20 观察 / 15 分钟）且全程审计。

**TCC 稳定性** — helper 运行于 `~/.crabagent/bin`（app bundle 之外），使用稳定的代码签名身份，辅助功能与屏幕录制授权跨重打包保留。

需要 macOS 13+；首次使用请在系统设置中给 CrabAgent 授予**辅助功能**和**屏幕录制**权限。
