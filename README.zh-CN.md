...
### 🖼️ 多模态
粘贴/拖拽图片到对话，自动检测模型是否支持视觉。AI 生成的图片可跨会话持久化，内联渲染。**懒加载**：图片在文本渲染后按需加载，打开图片密集的会话瞬间完成。

### 🗂️ 多工作空间
同时处理多个项目。工作空间切换器显示**实时徽章**，标识每个工作空间的活跃会话数量——无需来回切换查看后台 Agent 是否完成。当前工作空间中正在运行的会话用绿色脉冲指示点标记。
