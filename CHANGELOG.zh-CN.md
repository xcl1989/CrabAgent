...
### 新增
- **邮件智能任务提取** — LLM 自动分析邮件内容，从会议通知、截止日期、待办事项中提取并创建任务
- **任务↔对话关联** — 从邮件创建的任务自动关联到原始邮件对话，任务面板点击"查看详情"直达原文
- **通知面板优化** — 邮件通知展示原文预览、回信草稿和自动创建的任务列表，长内容支持展开/收起

### 修复
- **任务 API 路由未挂载** — `task_router` 导入后未注册到应用，导致前端任务面板始终为空
- **任务提取缺少 datetime 导入** — 导致邮件任务提取静默失败
- **通知展示截断** — 从单行截断改为智能预览 + 展开/收起
