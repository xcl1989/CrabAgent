...
| **定时任务** | 通过对话或 Web UI 创建定时任务，Agent 自主执行并推送通知 |
| **多 Agent 协作** | 委派任务给专门的 Agent（研究员、分析师、编程专家、写手）协同工作 |
...
CrabAgent 可以定时自主执行任务 — 你定义做什么和什么时候做，它自动执行并通知你。

### 创建任务

**通过对话** — 直接用自然语言告诉 Agent：
```
> 每天早上9点打开 Hacker News 把前5条新闻截图给我
> 每30分钟检查一次这个商品页面，价格低于500就通知我
```

Agent 会调用 `scheduled_task_create` 工具创建任务，默认使用当前对话的模型。

**通过 Web UI** — 点击侧边栏的 ⏱ Tasks 按钮，手动创建和管理任务。

### 可用工具

| 工具 | 说明 |
|------|------|
| `scheduled_task_create` | 创建任务（名称、问题、cron 表达式） |
| `scheduled_task_update` | 修改已有任务 |
| `scheduled_task_delete` | 删除任务 |
| `scheduled_task_list` | 列出所有任务及状态 |
| `scheduled_task_pause` | 暂停任务 |
| `scheduled_task_resume` | 恢复暂停的任务 |

### 通知

- 定时任务完成后，聊天头部右上角的铃铛图标会显示通知
- 点击通知可跳转到任务执行结果的对话
- 每 30 秒轮询一次，未读数量以角标显示

### 任务执行

- **自动审批**：工具调用无需确认（无人值守模式）
- **完整能力**：任务可访问浏览器、网页搜索、文件操作、MCP 等所有工具
- **结果存储**：每次执行创建新对话，包含完整消息历史和截图
- **错误追踪**：失败的任务会记录错误详情并通知你

---

## 多 Agent 协作

CrabAgent 支持多 Agent 协作 — 将任务委派给专门的 Agent 独立执行并汇报结果。

### 内置 Agent

| Agent | 角色 | 适用场景 |
|-------|------|----------|
| `researcher` 🔍 | 网络调研员 | 网页搜索、浏览、数据采集 |
| `analyst` 📊 | 数据分析师 | 数据对比、模式分析、报告生成 |
| `coder` 💻 | 编程专家 | 代码编写、审查、调试、重构 |
| `writer` 📝 | 内容写手 | 写作、编辑、翻译、格式化 |

### 委派任务

主 Agent 自动通过 `delegate_task` 工具创建子 Agent：

```
用户：对比我们的三个竞品

主 Agent → delegate_task("researcher", "浏览竞品A并总结")
主 Agent → delegate_task("researcher", "浏览竞品B并总结")
主 Agent → delegate_task("analyst", "对比分析结果并生成报告")

每个子 Agent 独立运行，拥有完整工具权限
结果由主 Agent 汇总后呈现给你
```

### 工作原理

- **隔离执行**：每个子 Agent 拥有独立的上下文、事件总线、工具集
- **自动批准**：子 Agent 无需用户确认，自动执行工具
- **事件流式**：子 Agent 的执行过程（文本、工具调用、完成）实时流式展示
- **折叠卡片**：UI 中以折叠卡片展示每个 Agent 的工作内容
- **禁止递归**：子 Agent 不能再次委派（防止无限循环）

### 管理 Agent

点击侧边栏的 🤖 Team 按钮，查看和自定义 Agent 的角色、目标和模型。
