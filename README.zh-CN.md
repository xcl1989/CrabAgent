...
### 🖼️ 多模态
可粘贴/拖拽图片到对话，也可让 Agent 通过 `read` 读取工作区已有图片。视觉模型会直接接收图片（大图自动缩放），普通图片查看不再绕行通用图片分析 MCP；OCR、UI、技术图、数据图、界面对比和视频等专业工具仍然可用。纯文本模型（如 kimi-k2、moonshot-v1）下图片自动降级为文本占位符，会话不中断。AI 生成的图片可跨会话持久化，内联渲染。**懒加载**：图片在文本渲染后按需加载，打开图片密集的会话瞬间完成。
