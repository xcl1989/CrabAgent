...
### 修复
- **空闲约 60 秒后实时更新停止到达（SSE"僵尸连接"）** — SSE 转发器会回收空闲超过 60 秒的连接队列，但存活时间戳只在事件真正转发时刷新。keepalive 由每条连接自己的消费循环产生、不经过转发器，因此"开着但空闲"的连接会被静默摘除订阅：客户端看起来在线、却再也收不到 `text_delta`/`tool_call` 等事件，界面直到运行结束、数据库重载一次性重绘才恢复。现在消费循环每次发出 keepalive 时都会刷新自己队列的时间戳，活跃连接不会被回收。全局事件流（`/events/global`，桌面宠物与通知在用）存在同样问题（120 秒阈值），已一并修复。
- **会话内容偶尔空白、需重新选择会话才显示** — 前端消息加载管线中，可变的当前会话引用被并发 effect（工作区自动加载、ref 同步）改写时会静默丢弃已取回的消息；工作区自动加载的慢响应会覆盖用户手动选择的会话；`agent_end`/monitor 的延迟重载可能在切换会话后把别的会话消息画到当前视图。现在所有消息加载由单调递增的加载令牌 + 当前会话校验统一仲裁，手动选择不可被覆盖，消息请求失败自动重试一次（如长任务期间 SQLite 忙导致的瞬时失败）。

### 变更
- 背景说明：此前移除空闲重连循环（每 ~40s 一次的"重新连接中"）揭开了僵尸连接 bug——频繁重连一直在无意中重新注册队列、恢复事件送达。现在两侧各归其位：重连只处理真正断开的连接，GC 只回收真正的死队列。

## [0.15.1] — 启动卡顿修复

### 修复
- 消除启动后卡死（Electron 启动路径、FTS 查询加固、调度器与 provider 存储健壮性）。

## [0.15.0] — 可信工作系统

### 新增
- 跨会话持久化工作跟踪：任务领域事件、聊天结果卡片、任务时间线与详情抽屉、通知铃等待标记。
- Agent 运行级恢复与重试；桌面宠物与工作区切换器接入持久化关注状态。
...
### 新增
- **直接读取本地图片** — 当前模型支持视觉时，内置 `read` 工具会把本地 PNG、JPEG、WebP、GIF、BMP 和 TIFF 图片嵌入下一次模型请求，不再只返回二进制文件信息。
- **图片缩放与格式归一化** — 过大或模型不直接支持的图片格式会先转换为受大小限制的 JPEG，再加入模型上下文，在保留视觉内容的同时控制请求体积。
- **多模态工具结果支持** — 内置工具和 MCP 工具可返回结构化文本及图片块；工具协议消息保持纯文本，图片通过符合协议要求的后续消息交给模型直接查看。

### 变更
- **原生视觉优先于通用 MCP 分析** — 使用视觉模型时，当前请求会隐藏通用 MCP `analyze_image` 工具，引导模型通过 `read` 和自身视觉能力读取图片；OCR、UI 提取、技术图、数据图、界面对比及视频等专业工具仍然保留。
- **本地化工具指引** — 中英文 `read` 工具描述均明确说明：已有图片路径时，应由视觉模型直接读取，而不是调用通用图片分析工具。

### 修复
- **能力降级可恢复** — 如果 Provider 实际拒绝多模态内容，即使所选模型被识别为视觉模型，CrabAgent 也会为当前执行持久化纯文本模式，并在重试时重新开放通用 MCP 图片分析能力。
- **浏览器截图传递** — 浏览器截图统一使用带大小控制的多模态处理流程，不再因超过原先 200 KB 的嵌入上限而被丢弃。
...
### 变更
- **浏览器成为聊天工作模式** — 协作浏览器不再是独立页面，不再自建会话管理、SSE 订阅和消息列表。它现在是聊天页的一种工作区类型，与文档编辑、代码编辑、会议记录并列。浏览器视图（地址栏、前进后退、刷新）在内容区展示，聊天面板统一处理会话上下文、消息、工具确认和输入。
- **共享会话上下文** — 在聊天页讨论需求后切到浏览器模式执行，保持同一个对话、消息历史和 Agent 上下文，不再有断裂的会话或跨会话消息泄露。

### 新增
- **页面版本安全** — 所有浏览器交互（点击、输入、滚动、选择）绑定 `collab_browser_observe` 返回的页面版本；陈旧页面的操作在 Electron Bridge 层被拒绝，强制重新观察后再执行。
- **敏感字段拦截** — 密码、隐藏、文件上传、OTP、CVV、支付字段在 Electron Bridge 层硬拦截，AI 无法输入。
- **高风险点击拦截** — 匹配支付、删除、发送、提交、转账的元素返回 `confirmation_required` 而非执行，通过 `collab_browser_wait_for_user` 路由到用户。
- **新浏览器工具** — `collab_browser_select`、`collab_browser_press_key`、`collab_browser_wait_for`（网络空闲 / 文本出现 / URL 匹配）。
- **浏览器任务持久化** — `BrowserTask` 和 `BrowserTaskEvent` 数据模型及 REST API，支持任务生命周期和脱敏审计时间线。
- **按浏览器任务筛选会话** — `GET /sessions?browser_only=true` 只返回拥有协作浏览器任务的会话。

### 移除
- **独立 BrowserCollaborationPage** — 旧的独立浏览器页面（含重复的会话/SSE/消息管理）已移除。所有浏览器协作现在通过统一聊天体验完成。
- **逐工具确认弹窗** — 所有协作浏览器工具移除 `requires_permission`。安全性在 Bridge 层强制执行，不再逐个动作打断用户。

### 修复
- **跨会话消息串流** — 消除了浏览器页独立 SSE 连接和会话状态可能收到其他会话事件的根因。
- **下拉菜单层级被原生网页遮挡** — 所有会话/工作空间/任务控件移入聊天侧边栏，消除了 DOM 菜单与原生 `WebContentsView` 的层级冲突。
...
### 修复
- **桌面端本地化系统提示词** — PyInstaller 桌面构建现在会打包运行时动态加载的 `en.json` 和 `zh-CN.json` 翻译资源；从 pip 已安装包构建桌面端时也会正确改写扁平目录布局中的 i18n 路径，避免本地化提示词退回英文或输出 `team_prompt.*` 原始键。
...
### 新增
- **用量洞察仪表盘** — Token 用量页现在以周期消耗、调用次数、缓存表现和活跃会话为主，并提供聚焦的趋势切换、模型与 Agent 排行，以及支持搜索、排序和展开摘要的会话诊断。

### 修复
- **一致的用量筛选** — 时间范围现在同时应用于概览指标和会话列表；趋势图 Y 轴预留足够标签空间，不再在左侧被裁切。
...
### 新增
- **完整文件工作区** — 工作模式文件列表支持在根目录或选中文件夹内新建文件夹、取消新建、使用 Cmd/Ctrl 或 Shift 多选、拖放批量移动，以及将项目移回工作区根目录。
- **文件操作与预览控制** — 恢复浮动右键菜单，可复制路径、下载、重命名、新建文件夹和删除；文本预览新增一键复制全部内容，`.xlsx`、`.docx`、`.pptx`、`.html` 和 `.md` 保留在工作区打开入口。

### 修复
- **目标完成兜底** — Agent 结束时若未记录终态，服务端会识别含验证证据的完成声明并自动关闭目标；不能可靠判定时则记录明确的收尾检查点，不再静默遗留过期的进行中状态。
- **更安全的文件移动** — 批量移动会在执行前校验空或重复来源、目标文件夹、名称冲突及目录移入自身等问题。
...
### 新增
- **目标模式** — 可通过靶心按钮或 `/goal <目标>` 创建会话级持久目标。目标会保存目标描述、验收条件、约束、检查点、状态历史、完成证据与具体阻塞原因；普通聊天输出仍完整保留在原会话中。
- **目标控制与进展时间线** — 对话界面新增目标卡片，支持暂停、继续、编辑和查看进展；进展视图会显示检查点、生命周期事件、完成证据和阻塞原因。
- **目标感知 Agent 工具** — 活动目标会注入 Agent 上下文。Agent 可读取目标、记录实质检查点，并且只有提供验证证据时才能完成目标；无法完成时必须提供具体 blocker。
- **受控自动继续** — 每次回复结束后，目标可以在独立会话回合中继续执行。除非用户另行配置，自动目标默认最多执行 10 轮、消耗 80,000 Token。

### 变更
- **目标生命周期同步到对话** — 目标创建、检查点、状态变化和自动续跑调度均通过 SSE 实时同步，并显示为会话中的可见提示。

### 修复
- **停止 Agent 时保留目标** — 中止执行中的目标任务会暂停目标并取消等待的续跑，不再悄然丢失任务目标。

---

## [0.13.1] — 可靠的上下文压缩与 Mermaid 流式渲染

### 新增
- **安全上下文压缩** — 压缩摘要前会将工具调用与工具结果转为普通文本上下文，保留工具的目的和结论，避免将供应商协议内容带入摘要；同时限制超大工具结果的长度。
- **保留最近对话轮次** — 持久化压缩会保留最新上下文，并将生成的摘要排在这些消息之前，使可见对话历史保持连贯。

### 修复
- **流式输出中的 Mermaid 语法错误 SVG** — Mermaid 图表会等待流式输出结束后再渲染，先校验语法，并将失败处理为正常提示，不再直接展示 Mermaid 的 `Syntax error in text` SVG。
- **压缩并发与排序问题** — 同一会话处理提示词时拒绝手动压缩请求；消息读取统一按对话序号排序。
...
### 新增
- **对话富可视化** — AI 回复现在可渲染 Mermaid 流程图、时序图、状态图、ER 图和架构关系图。仅在消息包含图表时才加载 Mermaid，保持首次打开对话的速度。
- **数据图表与 KPI 指标卡** — 新增 Markdown 围栏协议，支持柱状图、折线图、面积图、饼图、散点图和 KPI 摘要卡。图表支持放大、复制为 PNG，以及下载高清 PNG，便于直接分享。
- **可视化校验测试** — 前端 schema 测试覆盖标准图表、历史消息、自动推断分类字段、非法数据拦截和 KPI 解析。

### 修复
- **历史图表兼容** — 旧消息中 `series: ["字段"]` 与 `xField` 格式会自动规范化，并能与当前对象格式图表一起正常渲染。

## [0.12.7] — 叠影修复与图片工具增强

### 修复
- **精灵桌宠动画叠影** — `_align_frames` 方法使用 `alpha_composite` 进行帧对齐，它会叠加而非替换像素，导致半透明残影累积形成重影。已修复：在合成前正确清除画布再写入对齐后的帧。
- **ChatGPT 订阅下参考图生成** — `_image_edit` 现在自动识别 ChatGPT 订阅提供商，路由到专用的 `_chatgpt_image_edit` 方法，通过 Codex responses API 多模态 `input_image` + `image_generation` 工具让模型真正看到参考图。

### 新增
- **增强图片生成工具** — 新增参考图编辑（`image_edit` 工具）、多比例支持（1:1、4:3、3:2、16:9、9:16）、以及 `ref-preset` 模式自动保持原图风格。支持在保留构图的前提下用新提示词重新生成图片。
- **企业微信消息处理** — 新增 `wechat_message` 工具支持读取、回复和发送企业微信消息，支持消息轮询、附件处理和可配置发送间隔。
- **工具确认改进** — 确认对话框现在显示工具名称、执行次数并支持批量批准。`tool_confirm` 消息持久化，重连后可恢复确认状态。

### 变更
- **桌宠生成进度条** — 步骤标签现在显示每个动画行的详细中文描述（如"待机动画"、"向右跑动画"等），而非通用步骤编号。
- **前端静态资源缓存** — 所有前端构建产物文件名现在包含唯一内容哈希，避免缓存失效问题。
...
### 新增
- **AI 桌宠生成支持参考图** — 上传参考照片后，ChatGPT Codex 后端通过多模态 `input_image` 内容和 `image_generation` 工具接收参考图，生成的精灵图忠实还原参考角色的外貌、配色和风格。OpenAI API 提供商使用 `litellm.aimage_edit` 实现相同效果。
- **ChatGPT 额度面板重构** — 额度显示根据 API 返回的原始秒数动态格式化窗口大小（小时/天）和重置时间，自动适配未来的窗口变化（5 小时、7 天等），无需修改代码。

### 修复
- **桌宠生成按钮报错 `[object Object]`** — API 客户端将 `FormData` 对象做了 JSON 序列化，导致 multipart 上传失败。已修复：`ApiClient` 检测到 `FormData` 时跳过序列化且不设 Content-Type（浏览器自动加 multipart boundary）。非字符串的 `detail` 错误响应也正确转为字符串。
- **点击桌宠无法打开主窗口** — 按下时立即启动拖拽计时器，导致窗口微小位移（屏幕坐标与窗口坐标混算），污染了点击/拖拽检测。已修复：拖拽计时器仅在位移超过 4px 阈值后才启动。Sprite 桌宠点击后也会调用 `openMain()`（此前仅 SVG 桌宠有此行为）。
- **精灵桌宠左右晃动** — AI 生成的精灵图各帧角色位置不一致。已为静态动作行（待机、挥手、等待、工作、完成）增加交叉相关帧对齐，每帧相对第一帧做最优水平偏移修正，消除残余抖动。
- **精灵桌宠图像被裁剪** — Canvas 上的 `max-width/max-height: 100%` 在容器小于 192×208 帧时导致纵向压缩。已移除尺寸限制，并将 sprite 模式容器高度扩大到 208px。
- **Electron 桌宠窗口过小** — 窗口高度从 276px 增至 320px，完整容纳 192×208 精灵帧和状态气泡。

### 变更
- **桌宠风格选择器国际化** — 风格标签（像素、Q版、毛绒等）通过 i18n 系统支持中英文，不再硬编码英文字符串。
